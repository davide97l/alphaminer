...
def random_dataset(days: int = 252):
    ...
    df = pd.DataFrame({'open': open, 'close': close, 'date': date})
...
class TestFunction:
    ...
    def test_randn_functions(self, d):
        df = qlib_dataset()
        configs = [
            [randF._delay(df['OPEN'].values, d),
             f'Ref($open, {d})'],  # Qlib can read values outside of the scope of the dataset if available
            ...
            [
                randF._ts_sum(
                    randF._ts_stddev(randF._ts_mean(gpF.add2(df['OPEN'].values, df['HIGH'].values), d), d), d
                ), f'Sum(Std(Mean(Add($open, $high), {d}), {d}), {d})'
            ],
            [
                randF._ts_sum(randF._delay(randF._ts_argmax(df['LOW'].values, d) + 1, d), d),
                f"Sum(Ref(IdxMax($low, {d}), {d}), {d})"
            ],
        ]
    ...
    def test_nindata_functions(self, d):
        ...
        df[str(d)] = d
        configs = [
            [nindF._delay(df['OPEN'].values, df[str(d)]),
             f'Ref($open, {d})'],  # Qlib can read values outside of the scope of the dataset if available
            [gpF.add2(df['OPEN'].values, df['HIGH'].values), 'Add($open, $high)'],
            [gpF.mul2(df['OPEN'].values, df['HIGH'].values), 'Mul($open, $high)'],
            ...
            [
                nindF._ts_sum(
                    nindF._ts_stddev(
                        nindF._sma(gpF.add2(df['OPEN'].values, df['HIGH'].values), df[str(d)]), df[str(d)]
                    ), df[str(d)]
                ), f'Sum(Std(Mean(Add($open, $high), {d}), {d}), {d})'
            ],
            [
                nindF._ts_sum(nindF._delay(nindF._ts_argmax(df['LOW'].values, df[str(d)]), df[str(d)]), df[str(d)]),
                f"Sum(Ref(IdxMax($low, {d}), {d}), {d})"
            ],
        ]
        for config in configs:
    ...
    def test_joinquant_functions(self, d):
        df = qlib_dataset()
        pairs = [
            [gpF.add2(df['OPEN'].values, df['HIGH'].values),
             jqF._add(df['OPEN'], df['HIGH']).values],
            [gpF.sub2(df['OPEN'].values, df['HIGH'].values),
             jqF._sub(df['OPEN'], df['HIGH']).values],
            [gpF.mul2(df['OPEN'].values, df['HIGH'].values),
             jqF._mul(df['OPEN'], df['HIGH']).values],
            [gpF.div2(df['OPEN'].values, df['HIGH'].values),
             jqF._div(df['OPEN'], df['HIGH']).values],
            ...
            [randF._ts_delta(df['OPEN'].values, d),
             jqF._ts_delta(df['OPEN'], d).values],
            [randF._ts_argmax(df['OPEN'].values, d),
             jqF._ts_argmax(df['OPEN'], d)],
            [randF._ts_argmin(df['OPEN'].values, d),
             jqF._ts_argmin(df['OPEN'], d)],
            [randF._ts_max(df['OPEN'].values, d),
             jqF._ts_max(df['OPEN'], d).values],
            [randF._ts_min(df['OPEN'].values, d),
             jqF._ts_min(df['OPEN'], d).values],
            [randF._ts_mean(df['OPEN'].values, d),
             jqF._ts_mean(df['OPEN'], d).values],
            [randF._ts_stddev(df['OPEN'].values, d),
             jqF._ts_stddev(df['OPEN'], d).values],
            [randF._ts_corr(df['OPEN'].values, df['HIGH'].values, d),
             jqF._ts_corr(df['OPEN'], df['HIGH'], d).values],
            [randF._ts_cov(df['OPEN'].values, df['HIGH'].values, d),
             jqF._ts_cov(df['OPEN'], df['HIGH'], d).values],
        ]
        for p in pairs:
            randf_res = p[0]
            jqf_res = p[1]
