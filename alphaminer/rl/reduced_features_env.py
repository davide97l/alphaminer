...
class ReducedFeaturesEnv(TradingEnv):
    def __init__(self, model, load_path: str = None, flatten_obs: bool = False, len_index: int = None,
                 return_original_obs: bool = False, **kwargs):
        TradingEnv.__init__(self, **kwargs)
        self.model = model
        self.flatten_obs = flatten_obs
        self.return_original_obs = return_original_obs
    ...
    def reset(self) -> pd.DataFrame:
        ...
        if self.return_original_obs:
            assert not self.obs_compressed
            return [reduced_obs, obs]
        return reduced_obs
    ...
    def step(
        ...
        if self.return_original_obs:
            assert not self.obs_compressed
            return [reduced_obs, obs], reward, done, info
