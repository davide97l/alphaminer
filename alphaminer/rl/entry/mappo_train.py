...
main_config = dict(policy=dict(),
                   env=dict(n_evaluator_episode=8,
...
def get_policy_config(args,
    ...
    ppo_config = dict(cuda=True,
                      ...
                      collect=dict(n_episode=args.collect_env_num,
                      ...
                      model=dict(
                          ...
                          global_obs_shape={
                              'basic':
                              6 * (500 if not sample_size else sample_size),
                              '158':
    ...
    policy_config = deep_merge_dicts(policy_cls.default_config(),
                                     policy_config)
    policy_config.collect.collector = deep_merge_dicts(
        collector_cls.default_config(), policy_config.collect.collector)
    policy_config.eval.evaluator = deep_merge_dicts(
        evaluator_cls.default_config(), policy_config.eval.evaluator)
    policy_config.learn.learner = deep_merge_dicts(
        learner_cls.default_config(), policy_config.learn.learner)
...
def main(cfg, args):
    ...
    collect_env_cfg = get_env_config(args,
                                     market,
                                     train_start_time,
                                     train_end_time,
                                     env_cls=DingMATradingEnv)
    cfg.env.manager = deep_merge_dicts(
        SyncSubprocessEnvManager.default_config(), cfg.env.manager)

    policy_cfg = get_policy_config(args, PPOPolicy, EpisodeSerialCollector,
                                   InteractionSerialEvaluator, BaseLearner)
    ...
    # args.env_type = '158'
    # args.top_n = 20
    eval_env_cfg = get_env_config(args,
                                  market,
                                  eval_start_time,
                                  eval_end_time,
                                  env_cls=DingMATradingEnv)
    ...
    evaluator_env = SyncSubprocessEnvManager(
        env_fn=[
            lambda: evaluate_env_temp for _ in range(args.evaluate_env_num)
        ],
        cfg=cfg.env.manager,
    )
    ...
    tb_logger = SummaryWriter(
        os.path.join('./{}/log/'.format(args.exp_name), 'serial'))
    learner = BaseLearner(policy_cfg.learn.learner,
                          policy.learn_mode,
                          tb_logger,
                          exp_name=args.exp_name)

    collector = EpisodeSerialCollector(policy_cfg.collect.collector,
                                       collector_env, policy.collect_mode,
                                       tb_logger, args.exp_name)

    evaluator = InteractionSerialEvaluator(policy_cfg.eval.evaluator,
                                           evaluator_env,
                                           policy.eval_mode,
                                           tb_logger,
                                           exp_name=args.exp_name)
