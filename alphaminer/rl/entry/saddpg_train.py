...
def get_env_config(args, market, start_time, end_time, env_cls=DingTradingEnv):
    env_config = dict(
        ...
        end_date=end_time,
        ransom_sample=(args.env_type == 'sample'),
        strategy=dict(
...
def get_policy_config(args, policy_cls, collector_cls, evaluator_cls, learner_cls, buffer_cls=None):
    ddpg_config = dict(
        ...
        learn=dict(
            update_per_collect=args.collect_env_num * 2 * args.max_episode_steps * 5 // args.batch_size,
        ...
        collect=dict(
            n_sample=args.collect_env_num * 2 * args.max_episode_steps,
            collector=dict(),
        ),
        eval=dict(
            evaluator=dict(
...
def main(cfg, args):
    ...
    collect_env_cfg = get_env_config(args, market, train_start_time, train_end_time, env_cls=DingTradingEnv)
    eval_env_cfg = get_env_config(args, market, eval_start_time, eval_end_time, env_cls=DingTradingEnv)
    ...
    policy = DDPGPolicy(policy_cfg)

    collect_env_temp = DingTradingEnv(collect_env_cfg)
    evaluate_env_temp = DingTradingEnv(eval_env_cfg)
    ...
    while True:
        if evaluator.should_eval(learner.train_iter):
            ...
            if stop:
                break
        new_data = collector.collect(train_iter=learner.train_iter, drop_extra=False)
