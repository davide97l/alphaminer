...
def test_gtja_env():
    ...
    def mock_qlib_features(codes, *args, **kwargs):
        ...
        df = df[(df["datetime"] >= "2019-01-01") & (df["datetime"] <= "2019-01-10")]
    ...
    with patch.object(D, "features",
                      mock_qlib_features), patch.object(D, "list_instruments",
        ...
        for _ in range(5):
            ...
            assert (obs.index == init_codes
                    ).sum() > 490  # Stocks delisted during this period will be replaced by other stocks
        assert done
